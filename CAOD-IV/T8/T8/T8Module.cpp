...
private:
    ...
    // Увеличение емкости массива (выделяемая память)
    void increaseCapacity() {
        // Увеличиваем емкость массива в два раза и вычитаем 1, если текущая емкость больше или равна (2^10) - 1
        capacity = (capacity >= (1 << 10) - 1) ? (capacity += capacity *0.05) : (capacity * 2);
    ...
    void decreaseCapacity() {
        ...
        if (size < capacity / 2) {
            // Уменьшаем емкость массива на 1 и делим ее пополам
...
public:
    ...
    void reserve(size_t newCapacity) {
        ...
        // Копирование существующих элементов в новую память//memcpy
        for (size_t i = 0; i < size; ++i) {
            newData[i] = data[i];
        }
