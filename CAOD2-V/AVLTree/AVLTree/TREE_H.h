...
// Каждый класс должен иметь одну ответственность и изменяться только по одной причине

// O - Open/Closed Principle (Принцип открытости/закрытости)
// Классы должны быть открыты для расширения, но закрыты для модификации

// L - Liskov Substitution Principle (Принцип подстановки Лисков)
// Объекты подклассов должны быть взаимозаменяемы с объектами базового класса без нарушения функциональности
...
public:
    ...
    /*TreeNode<long>* create10000000000ElementsTree() {
        BinarySearchTree<long> tree;
        long mid = 10000000000 / 2;
        tree.Linsert(mid);  // Вставляем центральный элемент как корень

        for (size_t i = 0; i < 10000000000; i++) {
            if (i != mid) {
                tree.Linsert(i);  // Вставляем остальные элементы
            }
        }
    ...
    TreeNode<long>* createLargeTree(long n) {
        TreeNode<long>* root = nullptr;
        for (long i = 0; i < n; ++i) {
            root = insert(root, rand() % n);  // Вставляем случайные значения
        }
        return root;
    }

    TreeNode<long>* Linsert(TreeNode<long>* node, long value) {
        if (node == nullptr) {
            return new TreeNode<long>(value);  // Создаем новый узел, если его нет
        }

        TreeNode<long>* current = node;
        TreeNode<long>* parent = nullptr;

        // Итеративный процесс вставки без рекурсии
        while (current != nullptr) {
            parent = current;
            if (value < current->n_data) {
        ...
        if (value < parent->n_data) {
            parent->n_left = new TreeNode<long>(value);
        }
        else {
            parent->n_right = new TreeNode<long>(value);
        }
...
void runEmptyTreeTest() {
    ...
    auto testEmptyTreeStructure = [&](TreeNode<int>* root) {
...
//TODO Нужно, чтобы подставив что-то не int, чтобы срабатывал оператор (например data в узле) V

/// <summary>
/// Перегруженный оператор <
/// </summary>
/// <typeparam name="T">Тип данных в узле</typeparam>
/// <param name="lhs">Левый операнд (узел)</param>
/// <param name="rhs">Правый операнд (узел)</param>
/// <returns>Результат сравнения</returns>
template<typename T>
bool operator<(const TreeNode<T>& lhs, const TreeNode<T>& rhs) {
    return lhs < rhs;  // Сравнение данных
}
